-- Migration: Create shop_claim_requests table if it doesn't exist
-- This table is used by the API and admin dashboard for shop claim requests
...
ALTER TABLE shop_claim_files ENABLE ROW LEVEL SECURITY;

-- Step 5: Create basic RLS policies (allow service role to access)
-- Note: More restrictive policies can be added later if needed
DROP POLICY IF EXISTS "Service role can access all claims" ON shop_claim_requests;
CREATE POLICY "Service role can access all claims" ON shop_claim_requests
  FOR ALL USING (true);

DROP POLICY IF EXISTS "Service role can access all claim files" ON shop_claim_files;
CREATE POLICY "Service role can access all claim files" ON shop_claim_files
  FOR ALL USING (true);
