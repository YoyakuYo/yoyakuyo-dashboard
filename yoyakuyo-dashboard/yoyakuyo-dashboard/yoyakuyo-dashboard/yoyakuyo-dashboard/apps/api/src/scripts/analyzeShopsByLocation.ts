...
async function countShopsInLocation(location: { name: string; lat: number; lng: number }, radiusKm: number = 5): Promise<number> {
...
async function analyzeShopsByLocation() {
  console.log("🔍 Analyzing shops by location...\n");
  console.log(`📍 Checking ${JAPAN_LOCATIONS.length} locations\n`);
  console.log("=".repeat(80));
  
  const results: Array<{ location: string; count: number; index: number }> = [];
  let totalShops = 0;
  ...
  for (let i = 0; i < JAPAN_LOCATIONS.length; i++) {
    ...
    const count = await countShopsInLocation(location, radiusKm);
