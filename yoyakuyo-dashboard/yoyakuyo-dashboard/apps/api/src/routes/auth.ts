...
router.post("/signup-owner", async (req: Request, res: Response) => {
    try {
        ...
        const { data: existingById, error: checkByIdError } = await supabaseAdmin
            .from('users')
            .select('id, email')
            .eq('id', user_id)
            .maybeSingle();
        ...
        if (existingById) {
            ...
            const { data: existingByEmail, error: checkByEmailError } = await supabaseAdmin
                .from('users')
                .select('id, email')
                .eq('email', email)
                .maybeSingle();
            ...
            if (existingByEmail) {
                ...
                console.log('✅ User record recreated with correct user_id:', newUser?.id);
            } else {
        ...
        const { data: finalVerify, error: finalError } = await supabaseAdmin
            .from('users')
            .select('id')
            .eq('id', user_id)
            .maybeSingle();
        ...
        if (shop_name && shop_name.trim()) {
            ...
            if (shopError) {
                ...
                if (shopError.code === '23503' || shopError.message?.includes('foreign key constraint')) {
                    // Double-check user exists
                    const { data: emergencyCheck, error: emergencyError } = await supabaseAdmin
                        .from('users')
                        .select('id')
                        .eq('id', user_id)
                        .maybeSingle();
