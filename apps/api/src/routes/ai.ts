...
interface IntentClassification {
    intent: 'greeting' | 'booking_search' | 'other';
    confidence: number;
    keywords: string[];
}
...
function classifyIntent(message: string): IntentClassification {
    ...
    const greetingMatches = greetingKeywords.filter(keyword =>
        containsKeyword(lowerMessage, keyword.toLowerCase())
    );

    // Check for booking/search intent
    const bookingMatches = bookingSearchKeywords.filter(keyword =>
        containsKeyword(lowerMessage, keyword.toLowerCase())
    );
...
interface CustomerContext {
    name?: string;
    type: 'line_customer' | 'web_customer' | 'guest';
    language?: string;
}
...
async function generateGreetingResponse(
    ...
    const helpQuestion = await generateMultilingualResponse('how_can_i_help', languageCode);
    ...
    if (threadId) {
        try {
            ...
            if (threadData?.customer_email) {
                ...
                } else if (req.headers['user-agent']?.includes('LINE') || req.body.lineUserId) {
...
router.post("/chat", async (req: Request, res: Response) => {
    try {
        ...
        if (userIntent.intent === 'greeting') {
            ...
            const greetingLanguageCode = await detectLanguage(finalMessage);

            // Get customer context for personalized greeting (no thread needed for greetings)
            const customerContext = await getCustomerContext(req, undefined, greetingLanguageCode);

            // Generate greeting response
            const greetingResponse = await generateGreetingResponse(customerContext, greetingLanguageCode);
            ...
            try {
                ...
                console.log('[AI] Database save failed for greeting, but continuing:', dbError?.message || dbError);
            }
...
router.post("/chat-thread", async (req: Request, res: Response) => {
    try {
        ...
        if (userIntent.intent === 'greeting') {
            ...
            const customerContext = await getCustomerContext(req, threadId, undefined); // languageCode not yet available

            // Detect language for greeting
            const greetingLanguageCode = await detectLanguage(message);
